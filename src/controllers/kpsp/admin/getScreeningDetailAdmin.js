...
// Get single screening detail for admin (no ownership check)
const getScreeningDetailAdmin = async (req, res) => {
    try {
        ...
        const mappedScreening = {
            ...
            child: screening.anak ? {
                fullName: screening.anak.namaLengkap,
                birthDate: screening.anak.tanggalLahir,
                userId: screening.anak.penggunaId,
                user: screening.anak.pengguna ? {
                    name: screening.anak.pengguna.nama,
                    email: screening.anak.pengguna.email
                } : null
            } : null,
        ...
        console.error('Error in getScreeningDetailAdmin:', error);
        res.status(500).json({ message: 'Server error', error: error.message });
    }
};

module.exports = getScreeningDetailAdmin;
